...
namespace JobInfo
{
    ...
    internal class XROGi_Net
    {
        ...
        public void XWinSocket_Init(string url)
        {
            //      this.url = url;
            ws_url = url.Replace("http:", "ws:") + "ChatHandler.ashx";
            service_url = url.Replace("ws:", "http:") + "GetUserInfo.asmx";
            WS_serverUri = new Uri(ws_url);    //"ws://jobinfo/xml/ChatHandler.ashx"
...
*/
   ...
   //         else
    //            throw new ChatDisconnectedException();
        ...
        async public Task WS_CloseConnect()
        {
            try
            {
        ...
        private async Task WS_BeginConnect()
        {
            try
            {
                ...
                if (ws_ClientWebSocket.State != System.Net.WebSockets.WebSocketState.Open)
                {
                    ...
                    await ws_ClientWebSocket.ConnectAsync(WS_serverUri, cts.Token);
                    b_Connected = true;
        ...
        private async Task WS4_BeginConnect(string _URL_Socket)
        {
            try
            {
                if (!WS_serverUri.AbsoluteUri.StartsWith(_URL_Socket))
                    XWinSocket_Init(_URL_Socket);
                ...
                if (ws_socket == null  )
                {
                    ...
                    Task<bool> bb = ws_socket.ConnectAsync(WS_serverUri.ToString(), true);
                //    serverUri = new Url( _URL_Socket);
                }
                else
                {
        ...
        internal async Task ConnectToToken(string _URL_Socket, User this_user, Device d)
        {
            try
            {
                //    await WS_BeginConnect();
                WS4_BeginConnect(_URL_Socket);
                //         TestSendDevice(d);
